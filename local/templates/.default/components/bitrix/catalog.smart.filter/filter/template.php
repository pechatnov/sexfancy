...
while($ob = $res->GetNextElement())
{
	...
	$colors[$arFields['PROPERTY_CODE_VALUE']] = CFile::GetPath($arFields['PROPERTY_IMG_VALUE']);
	$colors_name[$arFields['PROPERTY_CODE_VALUE']] = $arFields['NAME'];
}


$brands = [];
$filter = Array('IBLOCK_ID' => 9, 'ACTIVE' => 'Y');
$select = Array('ID', 'NAME', 'PROPERTY_VENDORID');
$ob = CIBlockElement::GetList(array(), $filter, false, false, $select);
while($res = $ob->GetNext())
{
	$brands[$res['PROPERTY_VENDORID_VALUE']] = $res['NAME'];
}
asort($brands);
$sortBrands = [];
		...
		foreach($arResult["ITEMS"] as $key=>$arItem)
		{
			...
			if(isset($arItem["PRICE"])):
				...
				<div class="bx_filter_parameters_box active">
					...
					<div style="display: none;" class="bx_filter_parameters_box_title" onclick="smartFilter.hideFilterProps(this)"><?=$arItem["NAME"]?></div>
					<div class="bx_filter_block">
						<div class="bx_filter_parameters_box_container">
							<div class="price_slider">
								...
								<div class="inner">
									<div style="display: none;" class="bx_filter_parameters_box_container_block">
									...
									<div class="current_interval"><input type="text" id="amount" readonly"></div>
									<div class="bx_ui_slider_track" id="drag_track_<?=$key?>">
										...
										<div style="display: none;" class="bx_ui_slider_part p2"><span><?=$price2?></span></div>
										<div style="display: none;" class="bx_ui_slider_part p3"><span><?=$price3?></span></div>
										<div style="display: none;" class="bx_ui_slider_part p4"><span><?=$price4?></span></div>
									...
									<div class="prices">
										<div class="item"><?=(int)$arItem["VALUES"]["MIN"]["VALUE"]?> ₽</div>
										<div class="item"><?=(int)$arItem["VALUES"]["MAX"]["VALUE"]?> ₽</div>
									</div>
								</div>
							</div>
						</div>
					...
					<div class="bx_filter_parameters_box_container">
							...
							<div class="block">
								<?foreach($arItem["VALUES"] as $val => $ar):?>
									...
									<a <?if($ar["CHECKED"]){?>class="active"<?}?> title="<?=$colors_name[$ar["VALUE"]]?>">
										...
										<span class="color"><img src="<?=$colors[$ar["VALUE"]]?>" alt="<?=$colors_name[$ar["VALUE"]]?>"></span>
									...
									<span class="bx_filter_input_checkbox">
										...
										<span class="bx_filter_param_text" title="<?=$ar["VALUE"];?>"><?=$ar["VALUE"];?><?
											if ($arParams["DISPLAY_ELEMENT_COUNT"] !== "N" && isset($ar["ELEMENT_COUNT"])):
												?>
											<span style="display: none;" data-role="count_<?=$ar["CONTROL_ID"]?>">
										...
										<div class="item <?if($ar["CHECKED"]){?>active<?}?>">
											...
											<span class="bx_filter_input_checkbox">
												<input
													...
													onclick="smartFilter.click(this)"
													/>
												<span class="bx_filter_param_text" title="Бестселлеры">Бестселлеры<?
													if ($arParams["DISPLAY_ELEMENT_COUNT"] !== "N" && isset($ar["ELEMENT_COUNT"])):
														?>
													<span style="display: none;" data-role="count_<?=$ar["CONTROL_ID"]?>">
														<? echo $ar["ELEMENT_COUNT"]; ?>
						...
						<?}elseif($arItem['NAME'] == 'vendor_id'){?>
							...
							<div class="block">
								...
								foreach($sortBrands as $val => $ar){
									if(!is_array($ar)){
										unset($sortBrands[$val]);
									}
								}
								$arItem["VALUES"] = $sortBrands;
								?>
								<?foreach($arItem["VALUES"] as $val => $ar):?>
									<?if($brands[$ar["VALUE"]]){?>
										<div class="item <?if($ar["CHECKED"]){?>active<?}?>">
										...
										<span class="bx_filter_input_checkbox">
											<input
												...
												onclick="smartFilter.click(this)"
												/>
											<span class="bx_filter_param_text" title="<?=$brands[$ar["VALUE"]]?>"><?=$brands[$ar["VALUE"]]?><?
												if ($arParams["DISPLAY_ELEMENT_COUNT"] !== "N" && isset($ar["ELEMENT_COUNT"])):
													?>
												<span style="display: none;" data-role="count_<?=$ar["CONTROL_ID"]?>">
													<? echo $ar["ELEMENT_COUNT"]; ?>
...
<script>
    ...
    foreach($arResult["ITEMS"] as $key => $arItem){
		...
		if($arItem["VALUES"]["MIN"]["HTML_VALUE"]){
			...
			$min_price_cur = $arItem["VALUES"]["MIN"]["HTML_VALUE"];
		}else{

			$min_price_cur = $arItem["VALUES"]["MIN"]["VALUE"];
		}

		if($arItem["VALUES"]["MAX"]["HTML_VALUE"]){

			$max_price_cur = $arItem["VALUES"]["MAX"]["HTML_VALUE"];
		}else{

			$max_price_cur = $arItem["VALUES"]["MAX"]["VALUE"];
		}
    }
    ?>
	$( function() {
		$( ".bx_ui_slider_track" ).slider({
			...
			min: <?=$min_price?>,
			max: <?=$max_price?>,
			values: [ <?=$min_price_cur?>, <?=$max_price_cur?> ],
		...
		$( "#amount" ).val($( ".bx_ui_slider_track").slider( "values", 0 )  + ' ₽ - ' + $( ".bx_ui_slider_track").slider( "values", 1 ) + ' ₽');
	} );
</script>
